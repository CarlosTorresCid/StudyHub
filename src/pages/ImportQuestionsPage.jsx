...
function validateBanco(preguntas) {
  ...
  if (asigIds.size > 1) {
    errors.push(
      `Las preguntas pertenecen a varias asignaturas: ${[...asigIds].join(', ')}. Todas deben tener el mismo asignaturaId.`
    );
  }
  ...
  preguntas.forEach((q, i) => {
    ...
    if (!q.tipo) {
      pErrors.push('falta tipo');
    } else if (!VALID_TIPOS.includes(q.tipo)) {
      pErrors.push(`tipo "${q.tipo}" no válido (acepta: ${VALID_TIPOS.join(', ')})`);
    }

    if (!q.origen) {
      pErrors.push('falta origen');
    } else if (!VALID_ORIGENES.includes(q.origen)) {
      pErrors.push(`origen "${q.origen}" no válido`);
    }
    ...
    if (pErrors.length) {
      errors.push(`${label}: ${pErrors.join('; ')}`);
    }
  });
...
export default function ImportQuestionsPage() {
  ...
  const subjectLabel = useMemo(() => {
    ...
    const s = publicLibrary.getSubjects().find(subject => subject.id === asignaturaId);
    ...
    <div className="mgmt-page">
      ...
      <div className="mgmt-header">
        <div>
          ...
          <p className="mgmt-subtitle">
            Importa un banco JSON y publícalo en la biblioteca local.
          </p>
        </div>
      </div>
      ...
      {!available && (
        <div className="mgmt-draft-notice">
          ...
          <span>
            La publicación de archivos solo está disponible durante el desarrollo local
            (<code>npm run dev</code>). En producción esta función no está disponible.
          </span>
        </div>
      )}
      ...
      <div className="mgmt-section">
        ...
        <details className="mgmt-json-example">
          <summary>Ver ejemplo mínimo de banco de preguntas válido</summary>
  ...
  "preguntas": [
    {
      "id": "casi-t1-test-001",
      "asignaturaId": "casi",
      "temaPrincipalId": "tema-1",
      "temaIds": ["tema-1"],
      "tipo": "test",
      "origen": "generada_ia",
      "enunciado": "¿Cuál es la opción correcta?",
      "opciones": [
        "Opción A",
        "Opción B",
        "Opción C"
      ...
      "respuestaCorrecta": 0,
      "explicacion": "La opción A es correcta.",
      "fuentes": [],
        ...
        {errores.length > 0 && (
          <div className="import-error-block">
            <div className="import-error-block-header">
              ⚠ Se han encontrado {errores.length} error{errores.length !== 1 ? 'es' : ''} en el banco de preguntas
            </div>
            ...
            {errores.length > 12 && (
              <div className="import-error-more">
                … y {errores.length - 12} error{errores.length - 12 !== 1 ? 'es' : ''} más.
              </div>
            )}
          </div>
        )}
            ...
            <div className="banco-preview-table">
              {banco.slice(0, 10).map((q, i) => (
                <div key={i} className="banco-preview-row">
                  ...
                  {q.requiereRevision && (
                    <span className="banco-badge-revision">revisión</span>
                  )}

                  {q.verificada && (
                    <span className="banco-badge-verificada">✓</span>
                  )}
                </div>
              ))}
            </div>
      ...
      {phase === 'confirming' && (
        <div className="banco-confirm-warning">
          ...
          <div className="banco-confirm-actions">
            <button type="button" className="btn btn-primary" onClick={doPublicar}>
              Confirmar y reemplazar
            </button>

            <button type="button" className="btn btn-ghost" onClick={() => setPhase('preview')}>
              Cancelar
            </button>
          </div>
        </div>
      ...
      {phase === 'done' && publishResult && (
        <div className="import-status success">
          <p>
            <strong>Banco de preguntas publicado en la biblioteca local.</strong>
          </p>
      ...
      <div className="mgmt-save-bar">
        {phase === 'preview' && available && (
          <button type="button" className="btn btn-primary" onClick={handlePublicarClick}>
